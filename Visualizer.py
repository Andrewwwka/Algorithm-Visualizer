...
class DrawInformation:
    ...
    def __init__(self, width, height, lst):
        ...
        self.set_list(lst)
        self.start_time = 0 # Add start_time to the class
...
def draw(draw_info, algo_name, ascending, elapsed_time=0):
    ...
    sorting_algos = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | H - Heap Sort ", 1, draw_info.BLACK)
    draw_info.window.blit(sorting_algos, (draw_info.width/2 - sorting_algos.get_width()/2, 75))

    if elapsed_time > 0:
        timer_text = draw_info.FONT.render(f"Time: {elapsed_time:.3f}s", 1, draw_info.BLACK)
        draw_info.window.blit(timer_text, (10, 10))
...
def draw_list(draw_info, color_positions={}, clear_bg=False):
    ...
    if clear_bg:
        ...
        # Draw the live timer during sorting
        if draw_info.start_time != 0:
            elapsed_time = time.time() - draw_info.start_time
            timer_text = draw_info.FONT.render(f"Time: {elapsed_time:.3f}s", 1, draw_info.BLACK)
            pygame.draw.rect(draw_info.window, draw_info.BACKGROUND_COLOR, (10, 10, 180, 30))
            draw_info.window.blit(timer_text, (10, 10))
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.key == pygame.K_r:
                ...
                sorting = True
                draw_info.start_time = time.time()
                elapsed_time = 0
